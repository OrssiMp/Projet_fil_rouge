    ...
    <BaseHero variant="ghost" size="lg" class="min-h-[70vh] flex items-center">
        ...
        <template #actions>
            ...
            <BaseForm 
                ...
                @focusout="handleBlur"
            >
                <div class="flex flex-col sm:flex-row gap-3 sm:gap-2 items-end sm:items-center bg-base-100 p-2 rounded-2xl border border-base-200 shadow-lg relative z-50">
                    
                    <div class="flex-grow w-full">
                        <BaseInput 
                            v-model="searchQuery" 
                            type="text" 
                            placeholder="Poste, compétences,Ville ,entreprise..."
                            icon="search" 
                            class="w-full" 
                            autocomplete="off"
                        />
                ...
                >
                    <div
                        v-show="isOpen && searchQuery.length > 1"
                        ...
                        <ul class="overflow-y-auto p-2 flex flex-col gap-1">
                            <template v-if="filteredResults.length">
                                <li v-for="result in filteredResults" :key="result.id">
                                    ...
                                    >
                                        ...
                                        >
                                            <i v-if="result.type === 'job'" class="fa-solid fa-briefcase"></i>
                                            <i v-else-if="result.type === 'company'" class="fa-solid fa-building"></i>
                                        </div>
                                        ...
                                        <div class="flex-1 min-w-0">
                                            ...
                                            <div class="flex flex-wrap items-center gap-x-2 gap-y-1 mt-1 text-[11px] font-semibold text-base-content/60">
                                                ...
                                                <span v-if="result.location" class="flex items-center gap-1">
                                                    <i class="fa-solid fa-location-dot text-[9px]"></i> {{ result.location }}
                                                </span>
                                            </div>
                                        </div>
                                    </router-link>
                            ...
                            <div v-else class="px-6 py-10 text-center flex flex-col items-center justify-center">
                                <div class="w-12 h-12 bg-base-200 rounded-full flex items-center justify-center mb-3">
                                    <i class="fa-solid fa-magnifying-glass-minus text-xl text-base-content/30"></i>
                                </div>
        ...
        <template #visual>
            <div class="w-full max-w-md xl:max-w-lg card-interactive shadow-2xl rounded-2xl">
                <BaseImage class="hidden lg:block" :src="heroImage" alt="Jeune femme congolaise souriante cherchant un emploi sur Mosalah"
                    ratio="5/5" priority rounded="2xl" />
            </div>
        </template>
    </BaseHero>
</template>
...
const handleSearch = () => {
    if (searchQuery.value.trim() !== '') {
        console.log('Recherche validée pour :', searchQuery.value);
