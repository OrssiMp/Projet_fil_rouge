<template>
  ...
  <div class="drawer lg:drawer-open bg-base-200/30 min-h-screen font-sans selection:bg-accent selection:text-white">
    ...
    <div class="drawer-content flex flex-col h-screen overflow-hidden">
      ...
      <header class="navbar bg-base-100/80 backdrop-blur-md border-b border-base-200 h-16 shrink-0 px-4 lg:px-8 z-40 sticky top-0 transition-all">
        ...
        <div class="flex-none flex items-center gap-2 lg:gap-4">
          ...
          <router-link to="/entreprise/dashboard/offres" class="btn btn-sm h-9 bg-accent hover:bg-accent/90 text-white border-none shadow-sm shadow-accent/20 rounded-lg px-4 hidden sm:inline-flex">
          ...
          <div class="dropdown dropdown-end ml-1" v-if="user">
            <label tabindex="0" class="btn btn-ghost btn-circle avatar btn-sm border border-base-200 ring-2 ring-transparent hover:ring-accent/30 transition-all">
            ...
            <ul tabindex="0" class="mt-3 z-[100] p-2 shadow-xl menu menu-sm dropdown-content bg-base-100 border border-base-200 rounded-xl w-56">
              <li class="menu-title px-4 py-2">
                ...
                <span class="font-bold text-base-content truncate">{{ user.name }}</span>
              </li>
              <div class="divider my-0.5"></div>
              <li>
                <router-link to="/entreprise/settings" class="py-2.5 font-medium hover:text-accent">
                  <BaseIcon name="profil" class="w-4 text-center mr-2 text-base-content/40" /> Mon Profil
                </router-link>
              </li>
              <li>
                <router-link to="/entreprise/settings" class="py-2.5 font-medium hover:text-accent">
                  <BaseIcon name="parametres" class="w-4 text-center mr-2 text-base-content/40" /> Préférences
                </router-link>
              </li>
              <div class="divider my-0.5"></div>
              <li>
                <button @click="handleLogout" class="py-2.5 font-bold text-error hover:bg-error/10 hover:text-error">
                  <BaseIcon name="logout" class="w-4 text-center mr-2" /> Déconnexion
                </button>
              </li>
            </ul>
          </div>
    ...
    <div class="drawer-side z-50">
      ...
      <aside class="bg-base-100 border-r border-base-200 w-72 min-h-screen flex flex-col justify-between shadow-2xl lg:shadow-none transition-transform">
        
        <!-- En-tête Sidebar (Logo Produit & Marque) -->
        <div class="h-16 flex items-center justify-between px-6 border-b border-base-200 shrink-0 sticky top-0 bg-base-100 z-10">
          <div class="flex items-center gap-2">
            ...
            <span class="badge badge-sm bg-accent/10 text-accent font-black border-none text-[9px] tracking-wider uppercase ml-1">Recruiter</span>
          </div>
        ...
        <div class="flex-1 overflow-y-auto py-4 px-3 custom-scrollbar space-y-6">
          ...
          <div>
            <h3 class="px-3 text-[10px] font-black uppercase tracking-widest text-base-content/40 mb-2">Recrutement</h3>
              ...
              <li v-for="(item, index) in recruitmentMenu" :key="'rec-'+index">
                ...
                >
                  <div class="flex items-center gap-3">
                    <BaseIcon :name="item.icon" class="w-5 text-center text-lg transition-colors group-hover:scale-110 opacity-70" />
          ...
          <div>
            <h3 class="px-3 text-[10px] font-black uppercase tracking-widest text-base-content/40 mb-2">Communication</h3>
            <ul class="menu px-1 gap-0.5 text-sm font-semibold">
          ...
          <!-- SECTION 3 : Gestion -->
          <div>
            <h3 class="px-3 text-[10px] font-black uppercase tracking-widest text-base-content/40 mb-2">Gestion</h3>
            <ul class="menu px-1 gap-0.5 text-sm font-semibold">
        ...
        <div class="p-4 border-t border-base-200 bg-base-50 shrink-0">
          <div class="flex items-center justify-between group cursor-pointer p-2 -m-2 rounded-xl hover:bg-base-200 transition-colors">
            ...
            <div class="flex items-center gap-3 overflow-hidden">
              ...
              <div class="w-10 h-10 rounded-xl bg-accent text-white flex items-center justify-center shrink-0 shadow-inner font-black text-sm">
              ...
              <div class="min-w-0 flex-1">
                ...
                <span class="badge badge-sm badge-outline border-accent/30 text-accent font-bold text-[9px] uppercase tracking-wider">{{ company.plan }}</span>
              </div>
            </div>
...
const recruitmentMenu = [
  { label: "Tableau de bord", icon: "dashboard", to: "/entreprise/dashboard" },
  { label: "Mes offres", icon: "offres", to: "/entreprise/dashboard/offres", badge: "12", badgeColor: "bg-base-200 text-base-content" },
  { label: "Créer une offre", icon: "creer-offre", to: "/entreprise/dashboard/offres" },
  { label: "Tous les candidats", icon: "candidats", to: "/candidats" },
  { label: "Candidats reçus", icon: "inbox", to: "/entreprise/candidats", badge: "5", badgeColor: "bg-warning/20 text-warning-content" },
  { label: "Entretiens", icon: "entretien", to: "/entreprise/dashboard" },
];

// Section 2 : Communication
const communicationMenu = [
  { label: "Messages", icon: "messages", to: "/message", badge: "3", badgeColor: "bg-error/10 text-error" },
  { label: "Invitations", icon: "invitation", to: "/entreprise/dashboard" },
  { label: "Notifications", icon: "notifications", to: "/entreprise/dashboard" },
];

// Section 3 : Gestion
const managementMenu = [
  { label: "Collaborateurs", icon: "collaborateurs", to: "/entreprise/dashboard" },
