        ...
        <div class="flex-none gap-4">
          ...
          <div class="dropdown dropdown-end" v-if="currentUser">
            ...
            <ul tabindex="0" class="mt-3 z-[50] p-2 shadow-xl menu menu-sm dropdown-content bg-base-100 border border-base-200 rounded-2xl w-56">
              ...
              <li>
                <router-link to="/candidat/profile"><i class="fa-regular fa-user mr-2"></i> Mon Profil</router-link>
              </li>
              <li>
                <router-link to="/candidat/settings"><i class="fa-solid fa-gear mr-2"></i> Paramètres</router-link>
              </li>
              <div class="divider my-0"></div>
              <li>
                <button @click="handleLogout" class="text-error font-bold hover:bg-error/10">
    ...
    <div class="drawer-side z-[50]">
      ...
      <aside class="bg-base-100 border-r border-base-200 w-72 min-h-screen flex flex-col justify-between shadow-xl lg:shadow-none">
        <div class="overflow-y-auto pb-4 custom-scrollbar">
          ...
          <!-- Section 1 : Explorer -->
          <div class="px-6 py-4 text-[10px] uppercase tracking-widest text-base-content/40 font-black mt-2">
            Explorer
          </div>
          <ul class="menu px-4 gap-1 text-sm font-semibold">
            <li v-for="(item, index) in navigationMenu" :key="`nav-${index}`">
          ...
          <ul class="menu px-4 gap-1 text-sm font-semibold">
            <li v-for="(item, index) in trackingMenu" :key="`track-${index}`">
              <router-link :to="item.to" active-class="bg-accent/10 text-accent font-black">
                <i :class="item.icon" class="w-5 text-center opacity-70"></i>
                {{ item.label }}
                <span v-if="item.badge" class="badge badge-sm bg-accent text-white border-none ml-auto">
                  {{ item.badge }}
                </span>
              </router-link>
            </li>
          ...
          <!--
            Section "Mes Ressources" retirée : "Offres sauvegardées" et
            "Mes Documents" n'ont aucune donnée correspondante dans
            useDb/useAuth. À réactiver quand ces fonctionnalités existeront :
        ...
        <div class="p-4 border-t border-base-200 bg-base-50 shrink-0">
          <div class="flex items-center gap-3 px-2 mb-3">
            <div class="avatar placeholder">
              <div class="bg-accent text-white rounded-xl w-10 h-10 font-black text-sm shadow-inner flex items-center justify-center overflow-hidden">
                <img v-if="currentUser?.avatar" :src="currentUser.avatar" alt="Avatar" class="object-cover w-full h-full" />
...
const { fetchCandidaturesForCandidat } = useDb();
...
const loadSidebarCounters = async () => {
  ...
  const [candidatures, conversations] = await Promise.all([
    fetchCandidaturesForCandidat(currentUser.value.id),
    fetchConversations(currentUser.value.id, "candidat"),
  ]);
  const notifications = fetchNotifications(currentUser.value.id);

  myApplicationsCount.value = (candidatures || []).length;
  unreadMessagesCount.value = (conversations || []).reduce((acc, c) => acc + (c.unreadCount || 0), 0);
  unreadNotificationsCount.value = (notifications || []).filter((n) => !n.read).length;
};
...
const footerTagline = computed(() => {
  const a = currentUser.value?.availability;
  if (a === true) return "Disponible pour de nouvelles opportunités";
  if (typeof a === "string" && a) return a;
  return "À la recherche d'opportunités";
});

const handleLogout = () => logout();

const navigationMenu = [
  { label: "Les entreprises", to: "/entreprises", icon: "fa-solid fa-building" },
  { label: "Les offres d'emploi", to: "/offres", icon: "fa-solid fa-bullhorn" },
];

const trackingMenu = computed(() => [
  {
    label: "Mes candidatures",
    to: "/candidat/applications",
    icon: "fa-solid fa-paper-plane",
    badge: myApplicationsCount.value > 0 ? String(myApplicationsCount.value) : null,
  },
  {
    label: "Messages",
    to: "/message",
  ...
  },
  {
    ...
    badge: unreadNotificationsCount.value > 0 ? String(unreadNotificationsCount.value) : null,
  },
  // Retirés : aucune fonctionnalité d'entretien, d'invitation ou d'historique
  // n'existe encore dans useDb.
  // { label: "Entretiens", to: "/candidat/interviews", icon: "fa-regular fa-calendar-check" },
