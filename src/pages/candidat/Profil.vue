<template>
  ...
  <div v-else class="min-h-screen flex items-center justify-center">
    <p class="text-sm font-semibold text-base-content/50">Candidat introuvable ou chargement...</p>
  </div>
</template>
...
// 🗃️ Liste simulée contenant plusieurs profils (avec IDs distincts)
const mockCandidates = [
  {
    ...
    skills: ["JavaScript", "Vue.js 3", "Node.js", "Express", "Tailwind CSS", "Git", "npm"],
    languages: [
    ...
    experiences: [
      ...
      {
        period: "2024 - 2025",
        role: "Développeur Web Frontend (Stage)",
        company: "Agence Digitale Locale",
        location: "Brazzaville",
        description: "Intégration de maquettes responsives et développement d'interfaces dynamiques en équipe."
      }
    ],
    projects: [
      {
        ...
        description: "Développement d'un framework CSS ultra-minimaliste conçu pour accélérer l'intégration des interfaces responsives.",
        link: "#"
      },
      {
        name: "Chromalogger",
        tech: "JavaScript / npm package",
        description: "Utilitaire de journalisation léger permettant de formater proprement les logs dans les terminaux Node.js.",
        link: "#"
      }
    ]
  },
  {
    id: "2",
    name: "Nelly Mpierre",
    title: "Conceptrice UI/UX & Designer Web",
    avatar: "https://images.unsplash.com/photo-1494790108377-be9c29b29330?q=80&w=256&auto=format&fit=crop", 
    location: "Pointe-Noire, République du Congo",
    experienceYears: "3 ans",
    availability: "Sous préavis de 2 semaines",
    bio: `Créative et soucieuse de la clarté visuelle, je transforme des problématiques complexes en parcours utilisateurs élégants et intuitifs. Je maîtrise Figma ainsi que l'intégration front-end moderne via Tailwind CSS.`,
    skills: ["Figma", "UI/UX Design", "Wireframing", "Tailwind CSS", "Adobe Suite", "Prototypage"],
    languages: [
    ...
    experiences: [
      {
        period: "2024 - Présent",
        role: "Designer UI/UX UI Leader",
        company: "ImmoCool Services",
        location: "Brazzaville / Remote",
        description: "Refonte de la charte graphique globale et conception des parcours utilisateurs web et mobiles de la plateforme."
      }
    ],
    projects: [
      {
        name: "Mosalah Dashboard",
        tech: "Figma Component System",
        description: "Architecture de design d'un tableau de bord de recrutement complet avec plus de 40 variantes d'états réutilisables.",
        link: "#"
      }
    ]
  }
...
onMounted(() => {
  // 🔌 Extraction de l'ID depuis les paramètres d'URL (ex: /candidats/:id)
  const candidateId = route.params.id;
  
  // Recherche du profil correspondant
  const found = mockCandidates.find(c => c.id === String(candidateId));
  console.log(candidateId);
  
  if (found) {
    candidate.value = found;
  } else {
    console.warn(`[Mosalah] Aucun candidat trouvé pour l'ID: ${candidateId}. Retour au premier par défaut.`);
    // Fallback de sécurité : on charge le premier ou on redirige
    candidate.value = mockCandidates[0];
  }
});
</script>