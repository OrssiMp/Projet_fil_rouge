<template>
  <div class="min-h-screen bg-base-200/40 pb-16 select-none flex flex-col justify-between">
    ...
    <nav class="bg-base-100 border-b border-base-200 py-4 px-6 flex justify-between items-center">
      ...
      <BaseButton @click="$router.back()" variant="ghost" round shadow>
        Annuler ✕
      </BaseButton>
    </nav>
    ...
    <main class="flex-grow flex items-center justify-center px-4 py-12">
      <BaseCard v-if="job"
        class="w-full max-w-3xl overflow-hidden shadow-[0_4px_25px_rgba(0,0,0,0.02)] border border-base-200"
        density="spacious" data-aos="zoom-in">
        ...
        <BaseForm @submit="handleApply" class="flex flex-col gap-5">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <BaseInput v-model="form.email" label="EMAIL" type="email" placeholder="jean.dupont@email.com" icon="mail"
              disabled />
          </div>
          ...
          <div class="flex flex-col gap-2">
            ...
            <div @dragover.prevent="isDragging = true" @dragleave.prevent="isDragging = false"
              ...
              <div class="w-12 h-12 rounded-xl bg-base-200 flex items-center justify-center text-base-content/60">📄
              </div>
              <div v-if="!form.cvFile" class="text-sm font-medium text-base-content/80">
                <p>Glissez-déposez votre CV ici ou <span class="text-emerald-700 font-bold hover:underline">parcourez
                    vos fichiers</span></p>
          ...
          <div class="flex flex-col gap-2">
            <div class="flex justify-between items-center">
              <label class="text-xs font-black text-base-content/40 tracking-wider uppercase">Message au
                recruteur</label>
              <span class="text-xs font-medium text-base-content/40">(Optionnel)</span>
            </div>
            <textarea v-model="form.message" placeholder="Présentez brièvement vos motivations..."
              class="textarea textarea-bordered w-full h-32 rounded-2xl p-4 text-sm font-medium bg-base-100 border-base-300 focus:border-emerald-500 focus:outline-none transition-all resize-none"></textarea>
          </div>

          <template #actions>
            <div class="flex justify-end mt-4">
              <BaseButton type="submit" variant="accent" icon="send" class="w-full sm:w-auto px-8 shadow-md"
    ...
    <footer
      class="bg-base-100 border-t border-base-200 py-5 px-6 flex flex-col sm:flex-row justify-between items-center gap-2 text-xs md:text-sm font-semibold text-base-content/50">
      <span>Mosalah</span>
...
const form = reactive({
  fullName: currentUser.value.name || 'Jean Dupont',
  email: currentUser.value.email || 'jean.dupont@email.com',
  cvFile: null,
  message: ''
});
...
const handleApply = () => {
  if (isAuthenticated.value) {
    alert('Votre Candidature a été prise');
    router.push('/candidat/dashboard');
  } else {
    alert('Votre Candidature a été prise veuillez vous connecté afin de soumettre une candidature');
    router.push('/login');
  }
